export type ShellElements = {
  ...
  workspaceSidebarToggle: HTMLButtonElement;
  ...
  viewerModeCinematic: HTMLButtonElement;
  viewerModeTechnical: HTMLButtonElement;
  viewerControlRail: HTMLElement;
  viewerControlRailToggle: HTMLButtonElement;
  viewerTitle: HTMLElement;
  viewerHudTitle: HTMLElement;
  viewerHudCopy: HTMLElement;
  ...
  productInspectorToggle: HTMLButtonElement;
  productInspectorClose: HTMLButtonElement;
...
export function createShell(): ShellElements {
  ...
  <main class="shell">
    <header class="topbar">
      <div class="topbar-leading"><button id="workspace-sidebar-toggle" class="workspace-sidebar-toggle" type="button" aria-expanded="true" aria-controls="catalog-panel project-panel" aria-label="Collapse workspace sidebar" title="Collapse workspace sidebar">‹</button><div class="brand"><span class="brand-mark">CF</span><div><strong>CadFlow Harness</strong><span>local text-to-cad workspace</span></div></div></div>
    ...
    <section id="workspace" class="workspace">
      <aside id="catalog-panel" class="catalog-panel panel" aria-label="Project Catalog">
      ...
      <section id="project-panel" class="project-panel panel" aria-label="Current Project">
        ...
        <div id="project-content" hidden>
          ...
          <section class="run-metrics" aria-label="Agent Run metrics"><dl class="metric-grid metric-primary"><div><dt>Total Tokens</dt><dd id="total-tokens">--</dd></div><div><dt>Total Time</dt><dd id="run-time">--</dd></div><div><dt>Output Tokens</dt><dd id="output-tokens">--</dd></div></dl><details class="run-details"><summary>Run details</summary><dl class="metric-grid metric-secondary"><div><dt>Input Tokens</dt><dd id="input-tokens">--</dd></div><div><dt>Cached Tokens</dt><dd id="cached-tokens">--</dd></div><div><dt>Uncached Tokens</dt><dd id="uncached-tokens">--</dd></div></dl></details></section>
      ...
      <section class="viewer-panel" aria-label="CAD Viewer"><div class="viewer-workspace"><div id="viewer-stage" class="viewer-stage"><div class="viewer-heading"><div class="viewer-identity"><span class="eyebrow">INDUSTRIAL CAD WORKBENCH</span><strong id="viewer-title">Validated Result</strong><span class="viewer-subtitle">Parametric scene · Z-up workspace</span></div><div class="viewer-actions"><label id="preview-toggle-control" class="preview-toggle" hidden><input id="preview-toggle" type="checkbox" checked /><span>Live</span></label><button id="preview-retry" class="viewer-top-button" type="button" hidden>Retry</button><button id="fit-button" class="viewer-top-button" type="button">Fit</button><button id="product-inspector-toggle" class="viewer-top-button" type="button" aria-expanded="false" aria-controls="product-inspector">Details</button></div></div><div id="viewer" class="viewer"></div><div id="viewer-loading" class="viewer-loading" hidden><span class="spinner"></span><span>Loading Scene Artifact</span></div><div id="viewer-empty" class="viewer-empty"><span class="empty-mark">◇</span><strong id="viewer-empty-title">No Project selected</strong><span id="viewer-empty-copy">Select a Project to inspect its own Scene Artifact.</span></div>
            <aside id="viewer-control-rail" class="viewer-control-rail" aria-label="Viewport controls"><div class="viewer-rail-heading"><div><span class="eyebrow">VIEWPORT</span><strong>Scene controls</strong></div><button id="viewer-control-rail-toggle" class="icon-button" type="button" aria-expanded="true" aria-controls="viewer-control-rail" aria-label="Collapse scene controls" title="Collapse scene controls">−</button></div><section class="viewer-control-section"><h2>Display</h2><div class="viewer-mode" role="tablist" aria-label="Display mode"><button id="viewer-mode-cinematic" class="viewer-mode-button is-active" type="button" role="tab" aria-selected="true">Cinematic</button><button id="viewer-mode-technical" class="viewer-mode-button" type="button" role="tab" aria-selected="false">Technical</button></div></section><section class="viewer-control-section"><h2>Camera</h2><div class="viewer-control-grid"><button id="viewer-rotate" class="viewer-tool-button is-active" type="button" aria-pressed="true">Auto-Rotate</button><button id="viewer-reset" class="viewer-tool-button" type="button">Reset View</button></div></section><section class="viewer-control-section viewer-control-note"><h2>Scene state</h2><p>Selection outlines and isolation follow the accepted product structure.</p></section></aside>
            <div class="viewer-hud" aria-live="polite"><span class="viewer-hud-kicker">SCENE STATUS</span><strong id="viewer-hud-title">Awaiting validated artifact</strong><span id="viewer-hud-copy" class="viewer-hud-copy">Select a component to inspect its geometry and product data.</span></div><div class="viewer-hint">Drag to rotate · right-drag to pan · scroll to zoom</div><div class="viewer-command-dock" aria-label="Viewport commands"><span class="command-key">LMB</span><span>orbit</span><span class="command-key">RMB</span><span>pan</span><span class="command-key">WHEEL</span><span>zoom</span><span class="command-divider"></span><span class="command-accent">Z-UP</span></div><details id="preview-details" class="preview-details" hidden><summary>Preview diagnostics</summary><pre id="preview-log"></pre></details><div id="viewer-status" class="viewer-status" aria-live="polite"><span id="viewer-status-dot" class="status-dot"></span><span id="viewer-status-text">Waiting for a Validated Result</span></div></div>
          <aside id="product-inspector" class="product-inspector is-collapsed" aria-label="Accepted Product" hidden><div class="product-inspector-heading"><div class="product-heading-copy"><span class="eyebrow">ACCEPTED PRODUCT</span><strong id="product-title">Product</strong><span id="product-summary"></span></div><div class="product-heading-actions"><span id="product-status" class="product-status">Accepted</span><details id="product-downloads" class="product-downloads"><summary>Files</summary><div id="product-download-list" class="product-download-list"></div></details><button id="product-inspector-close" class="icon-button" type="button" aria-label="Close product details" title="Close product details">×</button></div></div><div class="product-tabs" role="tablist" aria-label="Product details"><button class="product-tab" type="button" role="tab" data-product-tab="structure">Structure</button><button class="product-tab" type="button" role="tab" data-product-tab="bom">BOM</button><button class="product-tab" type="button" role="tab" data-product-tab="validation">Validation</button><button class="product-tab" type="button" role="tab" data-product-tab="motion">Motion</button></div><div id="product-pane" class="product-pane" role="tabpanel" tabindex="0"></div></aside>
        </div>
      </section>
