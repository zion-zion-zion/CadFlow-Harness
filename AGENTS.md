...
This repository hosts a text-to-CAD agent workspace built around SimpleCADAPI. Python dependencies and project metadata live in `pyproject.toml`; exact versions are locked in `uv.lock`. Runnable CAD examples belong in `examples/`. The browser-based scene viewer is isolated in `viewer/`, with TypeScript source under `viewer/src/` and Vite configuration beside it. Agent reference material is packaged under `skills/simplecadapi/`. `backend/` is currently a scaffold; place future server modules there and mirror them with tests under `tests/`. Generated CAD files and viewer build output are ignored and should not be committed.
