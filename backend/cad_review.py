...
def _review_prompt(
    ...
    return (
        ...
        "Respect any explicit static-boundary instruction in the USER REQUEST and "
        "Design Contract. Do not fail because the source lacks a screw or universal "
        "joint coupling, motion sweep, force/friction/self-lock analysis, belt tension, "
        "fluid simulation, or disassembly animation when that capability is marked "
        "outside this benchmark stage. For a screw, accept separate revolute and "
        "prismatic joints plus pitch/travel metadata; for a universal joint, accept "
        "connector-frame phase metadata plus separate revolute joints. Only report a "
        "missing coupling when the requested coupling is explicitly supported and "
        "required by the static-boundary instructions. When the request permits "
        "primitive proxy geometry, judge unexposed installation coordinates and "
        "clearance details from explicit source parameters, connectors, constraints, "
        "and PRODUCT_SPEC metadata rather than inferring a defect from approximate "
        "render placement alone. A deterministically Passed product with those "
        "declared interfaces should not fail for visual proxy offsets that the host "
        "does not measure. Still fail a genuinely missing required component or "
        "constraint, an explicit numeric contradiction in the source, or any failed "
        "host validation gate. When checking primitive dimensions, account for the "
        "CadFlow replay API signature (`make_box_rsolid(width, height, depth, ...)`) "
        "and any clearly named helper that maps user-facing length/width/height "
        "arguments to it; do not treat argument order alone as a dimensional error. "
